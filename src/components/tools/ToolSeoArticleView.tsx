...
import { getToolSeoArticle, getToolSeoHeader } from '../../data/toolSeoArticles';
...
export const ToolSeoArticleView: React.FC<ToolSeoArticleViewProps> = ({
  ...
  const currentSlug = tool?.slug || toolSlug || '';
  const currentTitle = tool?.title || toolTitle || (currentSlug ? currentSlug.replace(/-/g, ' ').replace(/\b\w/g, l => l.toUpperCase()) : 'Craft Tool');

  const { heading: defaultHeading, subtitle: defaultSubtitle } = getToolSeoHeader(currentSlug, currentTitle);

  const [articleHtml, setArticleHtml] = useState<string>(() => sanitizePatternSeoHtml(getToolSeoArticle(currentSlug)));
  const [copied, setCopied] = useState<boolean>(false);

  useEffect(() => {
    const html = getToolSeoArticle(currentSlug);
    ...
    <div className="bg-white dark:bg-slate-900 rounded-3xl p-6 sm:p-8 border border-slate-200/80 dark:border-slate-800 shadow-sm space-y-6 my-8">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 border-b border-slate-100 dark:border-slate-800 pb-5">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-2xl bg-pink-50 dark:bg-pink-950/60 text-[#E96BA8] flex items-center justify-center font-bold shrink-0">
        ...
        <button
          onClick={handleShare}
          className="bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-200 px-4 py-2 rounded-2xl text-xs font-bold flex items-center gap-2 transition-colors cursor-pointer self-end sm:self-auto shrink-0"
        >
