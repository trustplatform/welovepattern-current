...
import { UsUkPatternConverterTool } from '../components/tools/UsUkPatternConverterTool';
...
export const ToolsView: React.FC<ToolsViewProps> = ({ initialToolId, onNavigate }) => {
  ...
  const renderActiveToolComponent = () => {
    ...
    switch (activeToolSlug) {
      ...
      case 'us-uk-crochet-pattern-converter':
        return <UsUkPatternConverterTool />;
      default:
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-200/80 dark:border-slate-800 pb-6">
        <div>
          ...
          <h1 className="text-2xl sm:text-3xl font-extrabold text-slate-900 dark:text-white tracking-tight">
            {activeToolSlug === 'us-uk-crochet-pattern-converter'
              ? 'US ↔ UK Crochet Pattern Converter'
              : 'Free Crochet Craft Tools & Calculators'}
          </h1>
          <p className="text-sm text-slate-500 dark:text-slate-400 mt-1 max-w-2xl">
            {activeToolSlug === 'us-uk-crochet-pattern-converter'
              ? 'Convert crochet patterns between US and UK terminology while preserving line breaks, stitch counts, and repeat instructions.'
              : 'Interactive calculators, row counters, size converters, and planning tools engineered specifically for yarn crafters. 100% offline-ready.'}
          </p>
        </div>
