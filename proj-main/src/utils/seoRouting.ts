...
export function parsePathname(
  ...
  if (path.startsWith('/pattern/')) {
    ...
    const exists = PATTERNS_DATA.some(p => p.slug === slug);
    if (!exists) {
...
export function generateSeoHead(
  ...
  const route = parsePathname(pathname, customBlogPosts, customCategories, customSitePages);
  ...
  if (route.isNotFound || route.view === 'not-found') {
    ...
    const pattern = PATTERNS_DATA.find(p => p.slug === route.patternSlug) || PATTERNS_DATA[0];
    ...
    if (tool) {
      title = `${tool.title} - Free Online Craft Tool | WeLovePattern`;
      description = `${tool.description} Free, interactive, and offline-capable ${tool.title.toLowerCase()} for crocheters and knitters.`;
      keywords = `${tool.title.toLowerCase()}, crochet ${tool.title.toLowerCase()}, ${tool.category.toLowerCase()} tool, free craft calculator, welovepattern`;
